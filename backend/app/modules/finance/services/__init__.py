...
class DebtService(BaseService):
    ...
    async def record_payment(
        ...
        *,
        ...
        # Resolve the payment method for its DB id and linked account code
        ...
        if pm.linked_account_code is None:
            raise BusinessRuleError(
                f"Payment method '{pm.name}' has no linked financial account and cannot be used for debt collection"
            )

        # Record the double-entry ledger effect using the financial account code
        ...
        # Update debt balance and status — round BEFORE comparing to avoid
        # sub-cent precision causing wrong PARTIAL vs PAID classification
        new_paid = round_money(debt.paid_amount + amount)
        if new_paid >= debt.original_amount:
            new_status = DebtStatus.PAID
            new_paid = debt.original_amount  # cap at original to avoid drift
        else:
