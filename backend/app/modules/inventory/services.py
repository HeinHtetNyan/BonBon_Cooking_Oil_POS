...
class InventoryService(BaseService):
    ...
    async def create_item(
        ...
        *,
        ...
        for attempt in range(1, 100):
            existing = await self._item_repo.get_by_code(final_code)
            if not existing:
                break
            final_code = f"{code}_{attempt}"
        else:
            raise ConflictError(f"Cannot generate unique code for '{code}'")
        code = final_code
        item = InventoryItem(
            code=code,
            name=name,
    ...
    async def delete_item(self, item_id: UUID, *, actor: str) -> None:
        item = await self._item_repo.get_by_id_or_raise(item_id)
        if item.current_balance != ZERO:
            raise BusinessRuleError("Cannot delete an item that has stock balance. Adjust balance to zero first.")
        await self._item_repo.soft_delete(item)
        self._logger.info("inventory.item_deleted", item_id=str(item_id), actor=actor)
